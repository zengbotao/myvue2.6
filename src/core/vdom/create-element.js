...
// 1.5createElement里核心的流程包括两个：
// 1.5.1 children的规范化
// - simpleNormalizeChildren：
// 1. 调用场景是 render 函数是编译生成的。
// 2. 理论上编译生成的 children 都已经是 VNode 类型的，但这里有一个例外，就是 functional component 函数式组件返回的是一个数组而不是一个根节点，
...
// 2. 另一个场景是当编译 slot、v-for 的时候会产生嵌套数组的情况，会调用 normalizeArrayChildren 方法；
  ...
  // 1.5.2规范完children后，需要创建一个VNode实例：

  
//   这里先对 tag 做判断：
// 1. 如果是 string 类型，则接着判断如果是内置的一些节点，则直接创建一个普通 VNode；
// 2. 如果是为已注册的组件名，则通过 createComponent 创建一个组件类型的 VNode，否则创建一个未知的标签的 VNode；
// 3. 如果是 tag 一个 Component 类型，则直接调用 createComponent 创建一个组件类型的 VNode 节点。
// 4. 对于 createComponent 创建组件类型的 VNode 的过程，本质上它还是返回了一个 VNode，后面讲解。
  let vnode, ns
